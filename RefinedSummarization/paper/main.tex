...
The four mechanism candidates point at three actionable fixes
(M1, M2, M3). We have implemented \textbf{M1} and \textbf{M2} as
prompt-only changes in the public repository under
\texttt{config/prompts.yaml} but \emph{have not yet validated them
empirically}. Validation is reserved for ongoing work.
...
\paragraph{(F2) New \texttt{coverage\_gap} issue type.}
We extend the verifier schema with a new \texttt{IssueType.COVERAGE\_GAP}
that fires when a fact appears in one or two of the three drafts and is
supported by paper evidence. The verifier is now told to prioritise
raising these issues, since they are the only positive signal the
refiner has to preserve draft-only specifics.

\paragraph{(F3) Paper-aware Step 0 (planned).}
A pre-extraction step that produces a list of $\sim$20 atomic ``must-cover''
paper facts before draft generation, conditioning all three drafters on
...
extraction. We have the implementation in place inside the evaluator;
porting it to the generation pipeline is a single-day change but
is reserved for the next iteration.

\paragraph{Hypothesis (untested).}
M1 and M2 should jointly be sufficient to recover most of the recall
deficit, since both DPR and FlashAttention case studies show the
information \emph{is} present in the underlying drafts. The expected
direction is $F_1$ improvement; we will not know the magnitude until we
re-run.
...
but hurts when many agents must aggregate breadth (union reconstruction).
The naive transfer between task types is the bug. We hope the result
informs the design of agent pipelines on long-form generation tasks
and motivates evaluation against single-call baselines under strict,
decoupled judges before claiming improvements.
