...
\midrule
\textsc{OneVision}      & $26$ & $0.584$ & $0.751$ & $0.485$ & $0.960$ & $0.438$ & $0.026$ \\
B1 Llama-naive          & $24$ & $0.503$ & $0.677$ & $0.408$ & $0.954$ & $0.363$ & $0.034$ \\
\textbf{B2 Qwen-naive}  & $24$ & $\mathbf{0.645}$ & $\mathbf{0.780}$ & $\mathbf{0.560}$ & $0.965$ & $\mathbf{0.517}$ & $\mathbf{0.015}$ \\
...
$n\!=\!29$ sample (each row reports the count of papers with valid
evals; drops are due to the recall checker exceeding its token
budget on PaLM-2-style long papers and the one PDF parse failure on
\texttt{2106.04561}). B2 (Qwen-72B-naive) and B3 (DeepSeek-V3-naive)
lead overall; \textsc{OneVision} is the strongest \emph{Llama-based}
method, sitting between B3 and B1 in mean $F_1$ and ahead of B6 (the
matched single-LLM control with the same prompt directives) by
$\geq 0.10$.}
...
\textbf{Method} & $n$ & $F_1$ & $F_{0.5}$ & $F_2$ & $P$ & $R$ & $\text{hal}$ \\
\midrule
\textsc{OneVision}      & $27$ & $0.515$ & $0.595$ & $0.466$ & $0.684$ & $0.445$ & $\mathbf{0.252}$ \\
B1 Llama-naive          & $23$ & $0.431$ & $0.529$ & $0.374$ & $0.661$ & $0.349$ & $0.289$ \\
\textbf{B2 Qwen-naive}  & $26$ & $\mathbf{0.559}$ & $\mathbf{0.598}$ & $\mathbf{0.533}$ & $0.642$ & $\mathbf{0.520}$ & $0.307$ \\
B3 DeepSeek-naive       & $27$ & $0.506$ & $0.546$ & $0.489$ & $0.596$ & $0.489$ & $0.353$ \\
B4 Llama-8B-naive       & $25$ & $0.407$ & $0.517$ & $0.345$ & $0.681$ & $0.316$ & $0.276$ \\
B5 Llama-structured     & $26$ & $0.383$ & $0.493$ & $0.323$ & $0.647$ & $0.295$ & $0.308$ \\
B6 Llama-v2prompt       & $26$ & $0.389$ & $0.474$ & $0.342$ & $0.592$ & $0.321$ & $0.368$ \\
\bottomrule
\end{tabular}
...
\multirow{2}{*}{\textbf{Baseline}} & \multirow{2}{*}{$n$}
  & \multicolumn{3}{c|}{\textbf{Strict-LLM}}
  & \multicolumn{3}{c}{\textbf{DeBERTa-NLI}}\\
& & $\Delta F_1$ & $95\%$~CI & sig.
  & $\Delta F_1$ & $95\%$~CI & sig. \\
\midrule
B1 Llama-70B-naive         & $24$ / $23$ & $+0.078$ & $[+0.010,+0.142]$ & ** & $+0.079$ & $[+0.034,+0.127]$ & ** \\
\textbf{B2 Qwen-72B-naive} & $24$ / $26$ & $-0.072$ & $[-0.136,+0.004]$ & ns\textsuperscript{$\dagger$}
                                          & $-0.044$ & $[-0.090,+0.007]$ & ns\textsuperscript{$\dagger$} \\
B3 DeepSeek-V3-naive       & $24$ / $27$ & $-0.041$ & $[-0.114,+0.034]$ & ns
                                          & $+0.009$ & $[-0.049,+0.066]$ & ns \\
B4 Llama-8B-naive          & $26$ / $25$ & $+0.128$ & $[+0.070,+0.185]$ & ** & $+0.106$ & $[+0.062,+0.151]$ & ** \\
B5 Llama-70B-struct        & $26$ / $26$ & $+0.153$ & $[+0.103,+0.205]$ & ** & $+0.131$ & $[+0.092,+0.172]$ & ** \\
\textbf{B6 Llama-70B-v2prompt} & $25$ / $26$ & $\mathbf{+0.106}$ & $\mathbf{[+0.047,+0.175]}$ & **
                                              & $\mathbf{+0.126}$ & $\mathbf{[+0.072,+0.187]}$ & ** \\
\bottomrule
\end{tabular}
\caption{Paired $\Delta F_1$ contrasts (\textsc{OneVision} $-$
baseline) on the $n\!=\!29$ sample with $95\%$ paired-bootstrap CIs
($10{,}000$ resamples). The $n$ column reports the strict / DeBERTa
paired counts (counts vary because some methods drop one or two
papers to recall-checker truncation on long papers). The boldfaced
row is the architecture-vs.-prompt control: B6 uses the \emph{same}
prompt directives as \textsc{OneVision}'s drafters and the same
Llama-70B backbone, but in a single-call, no-debate setting. The
$\sim\!+0.11$ to $+0.13$ $F_1$ gap, significant under both decoupled
judges, isolates the architectural contribution.\\
$^\dagger$The strict-mode CI on the B2 contrast just touches zero
from below ($+0.004$ upper edge), so we cannot reject the null that
\textsc{OneVision} loses to Qwen-72B-naive at this sample size.
DeBERTa-mode CI also touches zero ($+0.007$), giving the same
verdict.}
\label{tab:contrasts}
\end{table}
