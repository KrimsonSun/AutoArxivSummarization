...
function parseSummary(text: string): ParsedSummary {
    ...
    const tldrMd = tldrMatch ? tldrMatch[1].trim() : null;
    ...
    for (let i = 1; i < sectionParts.length; i++) {
        ...
        const contentMd = sectionParts[i].slice(firstNewline + 1).trim();
        if (title && contentMd) {
            sections.push({ title, contentMd });
        }
    }

    return { tldrMd, sections };
}
...
export default function PaperDisplay({ paper }: PaperProps) {
    ...
    const { tldrMd, sections } = parseSummary(rawSummary);
    const hasStructure = tldrMd !== null || sections.length > 0;
                            ...
                            <div style={{
                                ...
                                <div>
                                    <p style={{
                                        ...
                                        {lang === 'zh' ? 'Orientation (简述)' : 'Too Long; Didn\'t Read (TL;DR)'}
                                    </p>
                                    <div
            ...
            {adjudicatorResult && (
                <div style={{ marginBottom: '2.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                        ...
                        <h3 style={{ fontSize: '1.2rem', margin: 0, color: '#f43f5e' }}>{lang === 'zh' ? '批判性分析与破局 (Adjudicator Analysis)' : 'Critical Analysis & Solutions (Adjudicator Analysis)'}</h3>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                                ...
                                <h4 style={{ margin: 0, color: '#f43f5e', fontSize: '0.95rem' }}>
                                    {lang === 'zh' ? '逻辑错配诊断:' : 'Logic Mismatch Diagnosis:'} {adjudicatorResult.mismatch.mismatch_type} 
                            ...
                            <div style={{ fontSize: '0.9rem', lineHeight: 1.6, color: 'var(--foreground)' }}>
                                <p style={{ margin: '0 0 0.5rem 0' }}><strong>{lang === 'zh' ? '断裂边：' : 'Broken Edge:'}</strong> {adjudicatorResult.mismatch.broken_edge}</p>
                                <p style={{ margin: 0 }}><strong>{lang === 'zh' ? '诊断原因：' : 'Diagnosis Reason:'}</strong> {lang === 'zh' ? adjudicatorResult.mismatch.reasoning : (adjudicatorResult.mismatch.reasoning_en || adjudicatorResult.mismatch.reasoning)}</p>
                            </div>
                        </div>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: 'var(--primary)' }}>
                                <GitMerge size={16} />
                                <h4 style={{ margin: 0, fontSize: '0.9rem' }}>{lang === 'zh' ? '论文架构解构' : 'Paper Architecture Deconstruction'}</h4>
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr auto 1fr', gap: '0.5rem', alignItems: 'center' }}>
                                ...
                                <div style={{ background: 'rgba(0,0,0,0.2)', padding: '0.75rem', borderRadius: '8px', fontSize: '0.8rem' }}>
                                    <div style={{ fontWeight: 600, color: '#a855f7', marginBottom: '0.25rem' }}>{lang === 'zh' ? '前提 (Premises)' : 'Premises'}</div>
                                    <ul style={{ margin: 0, paddingLeft: '1rem', color: 'var(--muted)' }}>
                                        {adjudicatorResult.graph.premises.map((p: any, i: number) => <li key={i}>{lang === 'zh' ? p.description : (p.description_en || p.description)}</li>)}
                                    </ul>
                                </div>
                                ...
                                {/* Processes */}
                                <div style={{ background: 'rgba(0,0,0,0.2)', padding: '0.75rem', borderRadius: '8px', fontSize: '0.8rem' }}>
                                    <div style={{ fontWeight: 600, color: '#3b82f6', marginBottom: '0.25rem' }}>{lang === 'zh' ? '过程 (Processes)' : 'Processes'}</div>
                                    <ul style={{ margin: 0, paddingLeft: '1rem', color: 'var(--muted)' }}>
                                        {adjudicatorResult.graph.processes.map((p: any, i: number) => <li key={i}>{lang === 'zh' ? p.description : (p.description_en || p.description)}</li>)}
                                    </ul>
                                </div>
                                <ArrowRight size={16} className="text-muted" />
                                ...
                                {/* Conclusions */}
                                <div style={{ background: 'rgba(0,0,0,0.2)', padding: '0.75rem', borderRadius: '8px', fontSize: '0.8rem' }}>
                                    <div style={{ fontWeight: 600, color: '#10b981', marginBottom: '0.25rem' }}>{lang === 'zh' ? '结论 (Conclusions)' : 'Conclusions'}</div>
                                    <ul style={{ margin: 0, paddingLeft: '1rem', color: 'var(--muted)' }}>
                                        {adjudicatorResult.graph.conclusions.map((p: any, i: number) => <li key={i}>{lang === 'zh' ? p.description : (p.description_en || p.description)}</li>)}
                                    </ul>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', marginTop: '1rem' }}>
                                ...
                                <h4 style={{ margin: 0, fontSize: '1rem' }}>{lang === 'zh' ? '建设性破局方案 (Solutions)' : 'Constructive Solutions'}</h4>
                            </div>
                                ...
                                {adjudicatorResult.solutions.map((sol: any, idx: number) => (
                                    <div key={idx} style={{
                                        ...
                                        <div style={{ fontWeight: 600, fontSize: '0.95rem', marginBottom: '0.5rem', color: 'var(--primary)' }}>
                                            {lang === 'zh' ? `方向 ${idx + 1}:` : `Direction ${idx + 1}:`} {lang === 'zh' ? sol.direction : (sol.direction_en || sol.direction)}
                                        </div>
                                        <div style={{ fontSize: '0.85rem', marginBottom: '1rem', lineHeight: 1.5 }}>
                                            {lang === 'zh' ? sol.proposed_method : (sol.proposed_method_en || sol.proposed_method)}
                                        </div>
                                        ...
                                        {sol.references && sol.references.length > 0 && (
                                            <div style={{ background: 'rgba(0,0,0,0.3)', padding: '0.75rem', borderRadius: '8px' }}>
                                                <div style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                                                    <Database size={12} /> {lang === 'zh' ? 'Pinecone 知识库推荐' : 'Pinecone Knowledge Base Recommendation'}
                                                </div>
                                                        ...
                                                        <div key={rIdx} style={{ fontSize: '0.8rem' }}>
                                                            ...
                                                            {r.recommendation_reason && (
                                                                <div style={{ marginTop: '0.4rem', color: '#10b981', fontSize: '0.75rem', fontWeight: 600, background: 'rgba(16, 185, 129, 0.1)', padding: '0.4rem 0.6rem', borderRadius: '6px', borderLeft: '3px solid #10b981' }}>
                                                                    💡 {lang === 'zh' ? '推荐理由:' : 'Recommendation Reason:'} {lang === 'zh' ? r.recommendation_reason : (r.recommendation_reason_en || r.recommendation_reason)}
                                                                </div>
                                                            )}
                                                        </div>
                                                    ))}
